...
flights %>% group_by(month) %>% summarize(md= mean(dep_delay, na.rm = TRUE)) %>% arrange(desc(md))
...
delays_df <- data.frame(flights %>% group_by(month) %>% summarize(md= mean(dep_delay, na.rm = TRUE)) %>% arrange(desc(md)))
plot(delays_df)

# In which airport were the average arrival delays the highest?
# Hint: you'll have to perform a grouping operation before summarizing your data
flights %>% group_by(origin) %>% summarize(md= mean(arr_delay, na.rm = TRUE)) %>% arrange(desc(md))
